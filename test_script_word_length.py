...
def main():
    print("STARTING THE TEST FOR THE WORD LENGTH PROGRAM")
    ...
    master_word_lenght.setProgram(partition_locations, program_type=2)
    ...
    intermediate_data = processed_text.map(lambda word:(len(word), 1))
    #process the intermediate data and reduce
    word_len_freq = intermediate_data.reduceByKey(lambda x,z:x+z)
    #switch positions of key and value and sort the words in decreasing order by frequency
    word_len_freq_sorted = word_len_freq.map(lambda word:(word[1], word[0])).sortByKey(False)
    #Now we need to switch positions of key and value one more time to have the following format: word, frequency
    word_len_freq_sorted = word_len_freq_sorted.map(lambda word:(word[1], word[0]))
    # create the output
    word_len_freq_sorted.saveAsTextFile(output_spark_dir)
    sc.stop()
    ...
    print("TEST WORD LENGTH PROGRAM SUCCESFULLY COMPLETED!!!")
