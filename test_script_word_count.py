...
def main():
    print("STARTING THE TEST FOR THE WORD COUNT PROGRAM")
    ...
    #if intermediate and outputs directories are present, delete them.
    ...
    master_word_count = ms(interm_dir, out_dir, worker_number=worker_num, faulty_worker=faulty_worker, fault_type=fault_type)
    master_word_count.setProgram(partition_locations, program_type=1)
    master_word_count.run()
    ...
    intermediate_data = processed_text.map(lambda word:(word, 1))
    #process the intermediate data and reduce
    word_freq = intermediate_data.reduceByKey(lambda x,z:x+z)
    #switch positions of key and value and sort the words in decreasing order by frequency
    word_freq_sorted = word_freq.map(lambda word:(word[1], word[0])).sortByKey(False)
    #Now we need to switch positions of key and value one more time to have the following format: word, frequency
    word_freq_sorted = word_freq_sorted.map(lambda word:(word[1], word[0]))
    # create the output
    word_freq_sorted.saveAsTextFile(output_spark_dir)
    sc.stop()
    ...
    print("TEST WORD COUNT PROGRAM SUCCESFULLY COMPLETED!!!")
